...
public abstract class XtextTest {
	...
	public XtextTest(String resourceRoot) {
		 ...
		 * 
		 * For resources directly in the classpath, you need a starting slash after
		 * 'classpath:/': - classpath://bla.txt
		 * 
		 * But if you wan't to point to something in a subfolder, the subfolder must
		 * occur directly after 'classpath:/': - classpath://subfolder
		 * 
	...
	//
	// protected FluentIssueCollection testFile(String fileToTest, String...
	// referencedResources) throws Exception {
	// String fileContent = loadFileContents(resourceRoot, fileToTest);
	// URI uri = URI.createURI(resourceRoot + "/" + fileToTest);
	// EObject result = helper.parse(fileContent, uri, resourceSet);
	// Resource r = resourceSet.getResource(uri, false);;
	// return new FluentIssueCollection(r, null, new ArrayList<String>());
	// }

	protected FluentIssueCollection testFile(String fileToTest, String... referencedResources) {
		LOGGER.debug("testing " + fileToTest + " in test method " + this.getClass().getSimpleName() + "."
	...
	protected Pair<String, FluentIssueCollection> loadAndSaveModule(String rootPath, String filename) {
		...
		List<Issue> result = provider.getResourceValidator().validate(r, CheckMode.ALL, null);
	...
	/**
	 * If called prior to #testFile, serialization will be performed, but the result
	 * is not expected to exactly match the input file.
	 */
	...
	/**
	 * If called prior to #testFile, parser warnings will be ignored. Errors will
	...
	/**
	 * Serialization will occur without formatting, hence the input model must not
	 * comply to formatting rules in order to succeed.
	 */
	...
	/**
	 * Text file comparison will ignore OS specific newlines by harmonizing expected
	 * and serialized text with Unix style newline.
	 */
