...
public class PagingParameter implements Serializable {
	...
	public static PagingParameter pageAccess(int pageSize, int page, boolean countTotalPages,
			int additionalResultPages) {
		if (page < 1) {
			throw new IllegalArgumentException("Page numbers are 1 based");
		}
		if (additionalResultPages < 0) {
			throw new IllegalArgumentException(
					"additionalResultPages min values is 0 (actual " + additionalResultPages + ")");
		}
	...
	public static PagingParameter getNextPage(PagedResult<?> result) {
		...
		// TODO be more clever, look to getAdditionalPages or getTotalPages and if
		// exceeded don't increment
		// TODO cache total count, countTotal set to false but remember real count from
		// previous result
	...
	public static PagingParameter getFirstPage(PagedResult<?> result) {
		...
		return new PagingParameter(0, result.getPageSize(), false, result.getAdditionalResultPages(),
				result.getPageSize());
	}
