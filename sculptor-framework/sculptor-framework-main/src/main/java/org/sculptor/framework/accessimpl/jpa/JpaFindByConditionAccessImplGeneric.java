 ...
 * </p>
 */
public class JpaFindByConditionAccessImplGeneric<T, R> extends JpaCriteriaQueryAccessBase<T, R>
		implements
			FindByConditionAccess2<R>,
			ExpressionConverter {
	...
	protected void prepareFetch(Root<T> root, QueryConfig config) {
		...
		if (fetchEager != null) {
			for (Property p : getFetchEager()) {
				String propFullName = p instanceof LeafProperty<?>
		...
		for (ConditionalCriteria criteria : conditionalCriterias) {
			if (Operator.FetchEager.equals(criteria.getOperator())) {
				...
				// fetchLazy is not supported actually, but we will remove from list of
				// fetchEager for firstLevel
			}
		...
		for (ConditionalCriteria criteria : conditionalCriterias) {
			if (Operator.OrderAsc.equals(criteria.getOperator())) {
				if (config.isDistinct()) {
					...
					// for distinct select, sort column have to be in fetch columns - otherwise DB
					// error
	...
	protected void prepareHaving(CriteriaQuery<R> criteriaQuery, Root<T> root, QueryConfig config) {
		...
		if (!havings.isEmpty()) {
			// criteriaQuery.having(getCriteriaBuilder().and(havings.toArray(new
			// Predicate[havings.size()])));
	...
	private Predicate preparePredicate(ConditionalCriteria criteria, boolean forceJoin) {
		...
		if (Operator.Select.equals(criteria.getOperator()) || Operator.GroupBy.equals(criteria.getOperator())
		...
		if (Operator.Equal.equals(operator)) {
			...
			return builder.equal(builder.upper(path.as(String.class)),
					((String) criteria.getFirstOperant()).toUpperCase());
			...
			return builder.like(builder.upper(path.as(String.class)),
					((String) criteria.getFirstOperant()).toUpperCase());
			...
			return builder.between((Expression<Comparable>) path, (Comparable) criteria.getFirstOperant(),
					(Comparable) criteria.getSecondOperant());
			...
			return builder.or(preparePredicate((ConditionalCriteria) criteria.getFirstOperant(), true),
			...
			return builder.and(preparePredicate((ConditionalCriteria) criteria.getFirstOperant(), false),
			...
			return builder.lessThan((Expression<Comparable>) path,
					(Expression<Comparable>) getPath(root, (String) criteria.getFirstOperant()));
		} else if (Operator.LessThanOrEqualProperty.equals(operator)) {
			return builder.lessThanOrEqualTo((Expression<Comparable>) path,
					(Expression<Comparable>) getPath(root, (String) criteria.getFirstOperant()));
		} else if (Operator.GreatThanProperty.equals(operator)) {
			return builder.greaterThan((Expression<Comparable>) path,
					(Expression<Comparable>) getPath(root, (String) criteria.getFirstOperant()));
		} else if (Operator.GreatThanOrEqualProperty.equals(operator)) {
			return builder.greaterThanOrEqualTo((Expression<Comparable>) path,
					(Expression<Comparable>) getPath(getRoot(), (String) criteria.getFirstOperant()));
			...
			return builder
					.isTrue(builder.function("ftsEquals", Boolean.class, (Expression<Comparable>) path, ftsQuery));
