...
public abstract class AbstractDomainObject implements Serializable, Cloneable {
	...
	/**
	 * Only "simple" types will be included in toString. Relations to other domain
	 * objects can not be included since we don't know if they are fetched and we
	 * don't wont toString to fetch them.
	 * <p>
	...
	/**
	 * Subclasses may override this method to include or exclude some properties in
	 * toString. By default only "simple" types will be included in toString.
	 * Relations to other domain objects can not be included since we don't know if
	 * they are fetched and we don't wont toString to fetch them.
	 * 
	 * @param field
	...
	/**
	 * Subclass will typically override this method and return the real natural key
	 * or UUID key. If it is not overridden this default implementation returns
	 * null, which means that equals and hashCode will be implemented by
