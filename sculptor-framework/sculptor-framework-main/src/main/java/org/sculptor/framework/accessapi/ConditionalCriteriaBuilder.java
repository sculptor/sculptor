...
public class ConditionalCriteriaBuilder<T> {
	...
	public static class RootBuilderImpl<T> implements ConditionRootLogic<T>, OrderBy<T>, Selection<T>, GroupBy<T> {
		...
		private void addCriteriaStack(List<ConditionalCriteria> critList,
				SimpleStack<ConditionalCriteria> criteriaStack, boolean havingStack) {
		...
		public ConditionalCriteria buildSingle() {
			...
			return whereCriteriaStack.size() > 0
					? whereCriteriaStack.peek()
					: havingCriteriaStack.size() > 0 ? havingCriteriaStack.peek() : otherCriteriaStack.peek();
		}
		...
		private void assertHaving() {
			Optional<ExpressionOperator> operator = operatorStack.asList().stream()
					.filter(o -> !o.equals(ExpressionOperator.LBrace)).findAny();
			if (braceCount != 0 && operator.isPresent()) {
				throw new IllegalStateException(
						"withProperty()/where()/having() mixed inside braces (" + (braceCount > 0
								? ("missing " + braceCount + " rbrace)")
								: (braceCount + " too many rbrace)")));
			}
		}
		...
		// @Override
		// public GroupBy<T> groupBy(Expression<T> property,
		// ConditionalCriteria.Function function) {
		...
		// @Override
		// public Selection<T> select(Expression<T> property,
		// ConditionalCriteria.Function function, String alias) {
				...
				if (currentOperator == ExpressionOperator.Or || currentOperator == ExpressionOperator.And) {
					...
					if (popCriteria.getOperator() == Operator.And && currentOperator == ExpressionOperator.And
						...
						if (popCriteria.getFirstOperant() instanceof List) {
							List<ConditionalCriteria> criteriaList = (List<ConditionalCriteria>) popCriteria
									.getFirstOperant();
		...
		private ConditionalCriteria addSameCriteria(ConditionalCriteria existingCriteria,
				ConditionalCriteria newCriteria) {
			ConditionalCriteria compositeCriteria;
