...
public abstract class JpaQueryAccessBase<T, R> extends JpaAccessBase<T> {
	...
	final public void performExecute() throws PersistenceException {
		...
		// there is no support for Tuple in JPQL, need an untyped query for jpql queries
		// with more than one result expression
		// TODO: refactoring
		Query query;
	...
	protected void prepareHints(Query query, QueryConfig config) {
		if (isCache()) {
			// TODO: add support for other jpa provider or let the provider handle the
			// caching (recommended)
