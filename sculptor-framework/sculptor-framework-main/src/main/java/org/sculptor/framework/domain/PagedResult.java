...
public class PagedResult<T> implements Serializable {
	...
	public PagedResult(List<T> values, int startRow, int rowCount, int pageSize, int totalRows,
			int additionalResultRows) {
		// Check arguments
		if (values == null) {
			throw new IllegalArgumentException("Result is empty");
		} else if (startRow == UNKNOWN && rowCount == UNKNOWN && pageSize == UNKNOWN && totalRows == UNKNOWN
				&& additionalResultRows == UNKNOWN) {
		...
		// Store values
		if (startRow == UNKNOWN && rowCount == UNKNOWN && pageSize == UNKNOWN && totalRows == UNKNOWN
				&& additionalResultRows == UNKNOWN) {
			this.values = values;
