...
public class MediaRepositoryTest extends AbstractDbUnitJpaTests {
	...
	public void testFindByNestedCondition() throws Exception {
		...
		List<ConditionalCriteria> conditionalCriteria = criteriaFor(Media.class)
				.withProperty(MediaProperties.physicalMedia().location()).eq("abc123").build();
		List<Media> found = mediaRepository.findByCondition(conditionalCriteria);
		assertEquals(1, found.size());
		assertEquals("Pippi L�ngstrump i S�derhavet", found.get(0).getTitle());
	}
	...
	public void testFindByConditionTuple() throws Exception {
		List<ConditionalCriteria> condition = criteriaFor(Media.class).select(MediaProperties.lastUpdated())
				.countDistinct().select(MediaProperties.lastUpdatedBy()).countDistinct().select(MediaProperties.id())
				.countDistinct().build();
	...
	public void testFindByConditionStat() throws Exception {
		List<ConditionalCriteria> condition = criteriaFor(Media.class).withProperty(MediaProperties.id()).isNotNull()
				.build();
