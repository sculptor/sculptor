...
public class QueryConverterTest {
	...
	public void testSimpleConditions() {
		assertEquals(".withProperty(RootProperties.name().first()).eq(RootProperties.name().last())",
				new QueryConverter.ConditionalCriteriaStrategy("name.first = name.last", "Root").toQueryDsl());
		assertEquals(".withProperty(RootProperties.name().first()).eq(\"a and b\")",
				new QueryConverter.ConditionalCriteriaStrategy("name.first = 'a and b'", "Root").toQueryDsl());
		assertEquals(".withProperty(RootProperties.name().first()).eq(first)",
				new QueryConverter.ConditionalCriteriaStrategy("name.first = :first", "Root").toQueryDsl());
		assertEquals(".not().withProperty(RootProperties.name().first()).eq(first)",
				new QueryConverter.ConditionalCriteriaStrategy("name.first != :first", "Root").toQueryDsl());
	}

	@Test
	public void testBetween() {
		assertEquals(".withProperty(RootProperties.salary()).between(first,last)",
				new QueryConverter.ConditionalCriteriaStrategy("salary between :first and :last", "Root").toQueryDsl());
	}

	@Test
	public void testIn() {
		assertEquals(".withProperty(RootProperties.name().first()).in(first)",
				new QueryConverter.ConditionalCriteriaStrategy("name.first in (:first)", "Root").toQueryDsl());
		assertEquals(".withProperty(RootProperties.name().first()).in(\"A\",\"B\")",
	...
	public void testAnd() {
		assertEquals(
				...
				new QueryConverter.ConditionalCriteriaStrategy("salary is not null and salary <= :first", "Root")
						.toQueryDsl());
	}
	...
	public void testOr() {
		assertEquals(
				...
				new QueryConverter.ConditionalCriteriaStrategy("salary is null or salary2 is null or salary = salary2",
						"Root").toQueryDsl());
	}
	...
	public void testSelect() {
		assertEquals(
				...
				new QueryConverter.ConditionalCriteriaStrategy("select name, name.first,max(salary) as s", "Root")
						.toQueryDsl());
	}
	...
	public void testSelectWithFrom() {
		assertEquals(
				".select(RootProperties.name()).select(RootProperties.name().first()).select(RootProperties.salary()).alias(\"s\").max()",
				new QueryConverter.ConditionalCriteriaStrategy(
						"select r.name, r.name.first, max(r.salary) as s from Root r", "Root").toQueryDsl());
	}

	@Test
	public void testGroupBy() {
		assertEquals(".groupBy(RootProperties.name().first()).groupBy(RootProperties.test())",
	...
	public void testOrderBy() {
		assertEquals(
				...
				new QueryConverter.ConditionalCriteriaStrategy("order by name.first desc, test where test is null",
						"Root").toQueryDsl());
	}
}
